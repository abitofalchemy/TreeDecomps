...
from isomorph_interxn import isomorph_infile_reduce
from isomorph_interxn import isomorph_check_production_rules_pair, isomorph_intersection_2dfstacked
...
def stack_prod_rules_bygroup_into_list(prs_flst):
	# type: (list) -> object
	""" Stack Production rules by group into a list

	:rtype:
	:param prs_flst: list of paths to .prs files
	:return: dataframe (pandas)
	"""
	if not(isinstance(prs_flst, list)):
		print ("!! Not a list")
		exit(1)
	...
	for r in dfrm.iterrows():
		...
		if r[1]['lhs'] not in seen_rules.keys():
			...
			if nx.is_isomorphic(G1, G2, edge_match=label_match):
				# print ' ',r[1]['rnbr'], r[1]['rhs'], '::', df1[df1['rnbr'] == seen_rules[r[1]['lhs']][0]]['rhs'].values
				if DBG: print ' <-curr', seen_rules[r[1]['lhs']][0], ':', \
				dfrm[dfrm['rnbr'] == seen_rules[r[1]['lhs']][0]][
	...
	for r in df1.iterrows():
		...
		if r[1]['lhs'] not in seen_rules.keys():
			...
			if nx.is_isomorphic(G1, G2, edge_match=label_match):
				# print ' ',r[1]['rnbr'], r[1]['rhs'], '::', df1[df1['rnbr'] == seen_rules[r[1]['lhs']][0]]['rhs'].values
				if DBG: print ' <-curr', seen_rules[r[1]['lhs']][0], ':', \
				df1[df1['rnbr'] == seen_rules[r[1]['lhs']][0]][
					'rnbr'].values
				ruleprob2sum[seen_rules[r[1]['lhs']][0]].append(r[1]['rnbr'])
			else:
				seen_rules[r[1]['lhs']].append(r[1]['rnbr'])
...
# 	if not(args['fire'] is None):
# 		can_the_intersection_fire(args)
# 	else:
# 		isomorphic_test_on_prod_rules(args['orig'], args['pathfrag'], gname)
#
#
# def get_parser():
...
# 	parser.add_argument('--fire', required=False, help='Can rules fire?')
# 	parser.add_argument('--pathfrag', required=False, help='Input dimacs tree path fragment')
# 	parser.add_argument('-verb', action='store_true', default=0, required=0, help='Verbose (dev log info)')
