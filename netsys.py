...
def eigenvector_multiples(graphs):
    ...
    return df
def print_degree_distrib(df,graph_name):
  print '...',graph_name
...
def graph_properties(orig, hstars, bters=None, krons=None, gname='', metrix=[]):
    '''
    Network properties:
    orig:   original graph
    hstars: hrg genereated graphs (list)
    btergs: BTER generated graphs (list)
    krongs: Kronecker Product generated graphs (list)
    metrix: network metrics list
    '''
    ...
    if 'degree' in metrix:
        ...
        hstars_Deg = degree_distribution_multiples(hstars)
        if 0:
          orig__Deg.to_csv('Results/orig__Deg_{}.tsv'.format(gname),sep='\t',header=None)
          hstars_Deg.mean(axis=1).to_csv('Results/hstars_Deg_{}.tsv'.format(gname),sep='\t',header=None)
        elif 0:
            print_degree_distrib(orig__Deg.mean(axis=1), 'Orig')
            print_degree_distrib(hstars_Deg.mean(axis=1), 'Hstars')
        else:
            orig__Deg.mean(axis=1).plot(ax=ax0, marker="o", linestyle="", markeredgecolor='None', alpha=0.9, label="Orig")
            hstars_Deg.mean(axis=1).plot(ax=ax0, marker="+", linestyle="",markeredgecolor='b', label="HRG")

        if bters is not None:
            bter_Deg = degree_distribution_multiples(bters)
            # print_degree_distrib(bter_Deg.mean(axis=1), 'BTERs')
            bter_Deg.mean(axis=1).to_csv('Results/bter_Deg_{}.tsv'.format(gname),sep='\t',header=None)
            if path.exists('Results/bter_Deg_{}.tsv'.format(gname)): print 'Results/bter_Deg_{}.tsv'.format(gname)
            bter_Deg.mean(axis=1).plot(ax=ax0, marker="s", linestyle="", markerfacecolor="g", markeredgecolor='w', alpha=0.9, label="BTER")

        if krons is not None:
            kron_Deg = degree_distribution_multiples(krons)
            print krons_Deg.head()
            kron_Deg.mean(axis=1).to_csv('Results/kron_Deg_{}.tsv'.format(gname),sep='\t',header=None)
            if path.exists('Results/kron_Deg_{}.tsv'.format(gname)): print 'Results/kron_Deg_{}.tsv'.format(gname)
        ...
        # orig__Deg.mean(axis=1).to_csv('Results/degree_orig_{}.tsv'.format(gname),sep='\t')
        # synth_Deg.mean(axis=1).to_csv('Results/degree_hrg_{}.tsv'.format(gname),sep='\t')
        #
        # #ax0.set_xscale('log')
        # #ax0.set_yscale('log')
        ...
        # df1.to_csv('Results/deg_dist_{}.tsv'.format(gname),sep='\t', header=None)
        # if path.exists('Results/deg_dist_{}.tsv'.format(gname)):
        #     print '... file written:','Results/deg_dist_{}.tsv'.format(gname)

    if 'hops' in metrix:
      print 'Hops'
      orig__Hop_Plot = hop_plot_multiples(orig)
      synth_Hop_Plot = hop_plot_multiples(hstars)
      if bters is not None:
          bters_Hop_Plot = hop_plot_multiples(bters)
          bters_Hop_Plot.mean(axis=1).plot(ax=ax1, marker="s", markerfacecolor="g", linewidth=0.5,
                                            markeredgecolor='w', alpha=0.9, label="BTER")

      orig__Hop_Plot.mean(axis=1).plot(ax=ax1,  marker="o", markerfacecolor="r",
                                      markeredgecolor='w', alpha=0.9, linewidth=0.5,  label="Orig")
      synth_Hop_Plot.mean(axis=1).plot(ax=ax1,  marker="+", markeredgecolor='b',alpha=0.9,
                                        markerfacecolor="b", linewidth=0.5,label="HRG")
    ...
    #   synth_Hop_Plot.min(axis=1).plot(ax=ax1, color='r', alpha=0.2)
      ...
      if 0:
          orig__Hop_Plot.mean(axis=1).to_csv('Results/hops_orig_{}.tsv'.format(gname),sep='\t')
          synth_Hop_Plot.mean(axis=1).to_csv('Results/hops_hrg_{}.tsv'.format(gname),sep='\t')

    if 'clust' in metrix:
      print 'Clustering Coef'
      orig__clust_coef = clustering_coefficients_multiples(orig)
      synth_clust_coef = clustering_coefficients_multiples(hstars)
      if bters is not None:
          bters_clust_coef = clustering_coefficients_multiples(bters)
          gb = bters_clust_coef.groupby(['k'])
          gb['cc'].mean().plot( ax=ax2,marker="s", linestyle="", markerfacecolor="g", markeredgecolor='w', alpha=0.9, label="BTER" )
          ax2.set_title('Avg Clustering Coefficient', y=0.9)


      gb = orig__clust_coef.groupby(['k'])
      gb['cc'].mean().plot(ax=ax2, marker="o", linestyle="",markerfacecolor="r", markeredgecolor='w', alpha=0.9, label="Orig")
      gb['cc'].mean().to_csv('Results/clust_orig_{}.tsv'.format(gname),sep='\t')

      gb = synth_clust_coef.groupby(['k'])
      gb['cc'].mean().plot(ax=ax2, marker="+", linestyle="",markeredgecolor='b', label="HRG")
      ax2.set_title('Avg Clustering Coefficient', y=0.9)
      gb['cc'].mean().to_csv('Results/clust_hrg_{}.tsv'.format(gname),sep='\t')
      ax2.set_yscale('log')
      ax2.set_xscale('log')
      ax2.set_xlim(1, int(ax2.get_xlim()[1]*0.75))

    if 'assort' in metrix:
      print 'Assortativity'
      orig__assort = assortativity_coefficients_multiples(orig)
      synth_assort = assortativity_coefficients_multiples(hstars)

      gb = orig__assort.groupby(['k'])
      gb[1].mean().plot(ax=ax3, marker="o", linestyle="", markeredgecolor='None', alpha=0.9, label="Orig")
      gb[1].mean().to_csv('Results/assort_orig_{}.tsv'.format(gname),sep='\t')

      gb = synth_assort.groupby(['k'])
      gb[1].mean().plot(ax=ax3, marker="+", linestyle="",markeredgecolor='b', label="HRG" )
      ax3.set_title('Assortativity', y=0.9)
      gb[1].mean().to_csv('Results/assort_hrg_{}.tsv'.format(gname),sep='\t')

      if bters is not None:
          bters_assort = assortativity_coefficients_multiples(bters)
          gb = bters_assort.groupby(['k'])
          gb[1].mean().plot(ax=ax3, marker="s", linestyle="", markerfacecolor="g",
          markeredgecolor='w', alpha=0.9, label="BTER" )
      ax3.set_yscale('log')
      ax3.set_xscale('log')
      ax3.set_xlim(1, int(ax3.get_xlim()[1]*0.75))

    if 'kcore' in metrix:
        print 'kcore_decomposition'
        orig__kcore = kcore_decomposition_multiples(orig)
        synth_kcore = kcore_decomposition_multiples(hstars)


        orig__kcore.plot(ax=ax4, marker="o", linestyle="", markeredgecolor='None', alpha=0.9, label="Orig" )
        synth_kcore.mean(axis=1).plot(ax=ax4, marker="+", linestyle="",markeredgecolor='b', label="HRG")
        ...
        orig__kcore.to_csv('Results/kcore_orig_{}.tsv'.format(gname),sep='\t')
        synth_kcore.mean(axis=1).to_csv('Results/kcore_hrg_{}.tsv'.format(gname),sep='\t')

        if bters is not None:
            bters_kcore = kcore_decomposition_multiples(bters)
            bters_kcore.mean(axis=1).plot(ax=ax4, marker="s", linestyle="", markerfacecolor="g",
            markeredgecolor='w', alpha=0.9, label="BTER")
        # ax4.set_yscale('log')
    ...
    if 'eigen' in metrix:
      ...
      synth_eigenvec = eigenvector_multiples(hstars)

      orig__eigenvec= orig__eigenvec.transpose()
      orig__eigenvec.plot(ax=ax5, marker="o", linestyle="", markerfacecolor="r",
                            markeredgecolor='None',   alpha=0.9, label="Orig")
      orig__eigenvec.mean(axis=1).to_csv('Results/eigenv_orig_{}.tsv'.format(gname),sep='\t')

      synth_eigenvec= synth_eigenvec.transpose()
      synth_eigenvec.mean(axis=1).plot( ax=ax5, marker="+", linestyle="",markeredgecolor='b',
                                        alpha=0.9, label="HRG")
      synth_eigenvec.mean(axis=1).to_csv('Results/eigenv_hrg_{}.tsv'.format(gname),sep='\t')


      if bters is not None:
          bters_eigenvec = eigenvector_multiples(bters)
          bters_eigenvec.mean(axis=1).plot(ax=ax5, marker="s", linestyle="", markerfacecolor="g",
          markeredgecolor='w', alpha=0.9, label="BTER")

      ax5.set_yscale('log')
      ax5.set_xscale('log')
      ax5.set_xlim(0.1, int(ax5.get_xlim()[1]*1.1))
    ...
    if 'gcd' in metrix:
      ...
      df_g = external_rage(orig[0],gname) # original graph
      for synthG in hstars:
        gcd_network = external_rage(synthG,gname)
      ...
      ax6.bar([1], gcd_hrg_mean, width=0.5, yerr=gcd_hrg_std, color="g",  label='HRG')# http://blog.bharatbhole.com/creating-boxplots-with-matplotlib/
      ...
      ax6.set_xlim(0, 5)
      with open ('Results/gcd_{}.tsv'.format(gname), 'w') as f:
        f.write('{}\t{}\n'.format(gcd_hrg_mean,gcd_hrg_std))
      if bters is not None:
          gcd_bter = []
          for bterG in bters:
            gcd_network = external_rage(bterG,gname)
            # rgfd =  tijana_eval_rgfd(df_g, gcd_network)  ## what is this?
            gcm_g = tijana_eval_compute_gcm(df_g)
            gcm_h = tijana_eval_compute_gcm(gcd_network)
            gcd_bter.append(tijana_eval_compute_gcd(gcm_g, gcm_h))

          gcd_bter_mean = np.mean(gcd_bter)
          gcd_bter_std  = np.std(gcd_bter)

          ax6.bar([2], gcd_bter_mean, width=0.5, yerr=gcd_bter_std, label="BTER")#
      ax6.legend()

    oufigname = '/tmp/outfig_{}.pdf'.format(gname)
    plt.savefig(oufigname, bbox_inches='tight')
    if path.exists(oufigname): print 'Output: ',oufigname
...
def external_rage(G,netname):
    ...
    if "Users" in os.path.expanduser('~').split('/'):
